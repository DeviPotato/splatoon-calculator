...
splatoonApp.controller('MainCtrl', function($scope) {
        ...
        if ($scope.mains.length < 3) {
            if ($scope.abilities[i].lockout) {
                /* 				$scope.ErrorMessage = 'Conflicts with equipped ' + $scope.abilities[i].slot + ' ability!';
                				$scope.toggleModal(); */
                $scope.alerts.unshift({
                    "msg": "Conflicts with equipped " + $scope.abilities[i].slot + " ability!"
                })
                return;
            }
    ...
    function calc() {
        ...
        var special = $.grep($scope.specials, function(e) {
            return e.name == $scope.selectedWeapon.special;
        })[0];
